 ...
 * 定义 KEY 编号对应的管脚
 */
PTXn_e KEY_PTxn[KEY_MAX] = {PTC18, PTD0, PTD2, PTD8, PTD9, PTD10};
 ...
 */
KEY_MSG_t get_key_msg(void)
{
    uint8 tmp;
    KEY_MSG_t this_key;
    this_key.key=key_msg[key_msg_front].key;       //从FIFO队首中获取按键值
    this_key.status=key_msg[key_msg_front].status; //从FIFO队首中获取按键类型
