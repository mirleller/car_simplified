...
//void     adc_start   (ADCn_Ch_e, ADC_nbit);    //开始adc转换
...
/*!
 *  @brief      启动ADC软件采样,硬件平均(不支持B通道)
 *  @param      ADCn_Ch_e    ADC通道
 *  @param      ADC_nbit     ADC精度（ ADC_8bit,ADC_12bit, ADC_10bit, ADC_16bit ）
 *  @param      ADC_nsample  ADC连续转换次数（ ADC_4sample，ADC_8sample，ADC_16sample，ADC_32sample ）
 *  @since      v5.0
 *  @note       启动后即可等待数据采集完成触发中断
 *  Sample usage:       adc_start(ADC0_SE10, ADC_8bit);
 */
void adc_start_hv(ADCn_Ch_e adcn_ch, ADC_nbit bit,ADC_nsample samples)
{
    ADCn_e adcn = (ADCn_e)(adcn_ch >> 5) ;
    uint8 ch = (uint8)(adcn_ch & 0x1F);
    ...
    ADC_SC3_REG(ADCN[adcn])   =(0
                                |ADC_SC3_ADCO_MASK              //连续转换使能，0为禁止，1为使能
                                |ADC_SC3_AVGE_MASK              //硬件平均使能，0为禁止，1为使能
                                |ADC_SC3_AVGS(samples)          //配置转换次数，4,8,16,32次可选
                               );
